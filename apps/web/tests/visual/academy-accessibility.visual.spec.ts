...
async function approvedRegions(page: Page): Promise<Region[]> {
  return page.locator(".books-note a, .certification-disclaimer").evaluateAll((nodes) => nodes.map((node, index) => {
    const rect = node.getBoundingClientRect();
    return {
      name: node.classList.contains("certification-disclaimer") ? "certification-disclaimer" : `books-note-link-${index + 1}`,
...
async function compareWithinApprovedRegions(baseline: Buffer, corrected: Buffer, regions: Region[]) {
  ...
  return { changedPixels, outsideApprovedRegions, regionPixels, diffPng };
}

for (const visual of states) {
  test(`accessibility-corrected ${visual.state} differs only in approved Academy regions`, async ({ page }, testInfo) => {
    ...
    for (const capture of [
      ...
      const reversal = await page.addStyleTag({ content: `
        .books-note a { text-decoration-line: none !important; }
        .certification-disclaimer { color: var(--ink-mute) !important; }
      ` });
      ...
      if (capture.fullPage) {
        expect(comparison.regionPixels["books-note-link-1"]).toBeGreaterThan(0);
        expect(comparison.regionPixels["books-note-link-2"]).toBeGreaterThan(0);
      } else {
      ...
      writeFileSync(`${correctedPath}.json`, `${JSON.stringify({
        ...
        classification: "Approved: persistent link underlines and minimally darkened certification disclaimer only.",
      }, null, 2)}\n`);
    }
  });
}
