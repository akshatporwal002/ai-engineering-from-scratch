...
- Current implementation commit: `6ca742c8` — `chore(platform): scaffold nextjs and fastapi apps`
...
| 2. Shared UI system | Unstarted | No visual migration or screenshots were produced. |
...
| 8. Evidence suite | Partial | Foundation unit tests and local proxy verification only; no Playwright, accessibility, responsive, or screenshot evidence. |
...
`apps/web/package.json` pins Next.js 15.5.23, React, Tailwind, TypeScript, Vitest, and component-test support. The initially selected Next.js 15.5.7 was replaced after npm reported a critical vulnerability. `apps/api/pyproject.toml` pins FastAPI, Pydantic, Pydantic Settings, and Uvicorn. All are isolated from lesson directories.
...
There are no migrated routes, so no visual-parity or accessibility claim is made. `STARTING_STATE.md` records the baseline. The security review scope was limited to request IDs, safe error envelopes, header allowlisting, and log redaction; later input, authentication, persistence, and provider boundaries remain unimplemented.
...
5. Begin Workstream 2 only with paired legacy/new screenshots, keyboard checks, and accessibility evidence as required by the runbook.

## Recommended next action

Keep the completed foundation commit. Resolve the legacy CI failure in a separate scoped investigation, then begin the shared UI system and evidence harness; do not migrate a public route until that gate is in place.
