...
- **Stage 2 — in progress:** the shared shell centralizes Codeology product/source configuration, navigation, footer ownership and design tokens; rebrands the public chrome; adds persistent upstream attribution; and enforces pure white/black canvases with rounded/translucent controls. The homepage leads with the learn/build/prove proposition and the representative lesson reader carries Codeology metadata plus a pinned-source badge. The imported glossary now has Codeology metadata and source framing while retaining all 243 terms, URL-backed search, filters, deep links, command palette, read-aloud and mobile navigation. Homepage, lesson and glossary desktop-light/mobile-dark baselines are stored. Deterministic validators cover provenance drift, navigation, semantics, links, structured data, safe source paths, live search, header clearance, baseline dimensions and AA/AAA token contrast. Catalog and skill-map baselines remain next.
