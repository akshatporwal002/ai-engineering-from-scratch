...
- **Stage 2 — in progress:** the shared shell centralizes Codeology product/source configuration, navigation and design tokens; rebrands the header; adds persistent upstream attribution; and enforces pure white/black canvases with rounded/translucent controls. The homepage now leads with the learn/build/prove product proposition, treats AI Engineering From Scratch as a credited free pathway, removes unsupported inherited social proof from the visible surface, and has stored desktop-light/mobile-dark baselines. The representative lesson reader now carries Codeology metadata plus an in-context badge linked to the exact imported source commit, with desktop-light/mobile-dark baselines and responsive drawer checks. Deterministic validators cover provenance drift, navigation, semantic structure, links, structured data, safe source paths, baseline dimensions and AA/AAA token contrast. Homepage and lesson responsive/theme checks pass without horizontal overflow. Glossary, catalog and skill-map baselines remain next.
