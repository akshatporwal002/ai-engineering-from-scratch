...
- **Stage 3 — in progress:** assessment charter version 1 publishes stable learned, practised, demonstrated and verified definitions, anchored rubric language, the open-tool policy, separate artifact/identity/administration assurance axes and a deterministic policy-engine boundary. The public assurance guide is validated against the machine-readable policy and honestly caps the current product at learning activity. Strict v1 pathway, skill, scenario, rubric and evidence schemas now enforce immutable references, acyclic skill graphs, job-task evidence before publication, public/assessor separation and honest assurance ceilings. Target-role job-task analysis and the first 8–12-skill pilot pathway remain next.
