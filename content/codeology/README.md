...
content/codeology/
  ...
  assessment-blueprints/ # Reviewed task-to-scenario-to-skill coverage maps
```
...
The v1 schema contract is documented in `docs/CODEOLOGY_CONTENT_SCHEMA.md` and enforced by `python scripts/audit_codeology_content.py`. Every published pathway references exact job-task-analysis and assessment-blueprint versions; both must pass source, coverage and reviewer gates. This prevents an implementation convenience from silently becoming a competency claim.
