...
(function () {
  ...
  document.getElementById('cvProviderModelSave').addEventListener('click', async function () { var provider = providers.find(function (item) { return item.provider === selectedProvider(); }) || null; var model = document.getElementById('cvProviderModel').value; if (!provider || provider.model === model) { status('cvProviderStatus', 'Choose a different model to update this connection.', 'neutral'); return; } setBusy(providerForm, true); status('cvProviderStatus', 'Verifying the selected model…', 'neutral'); try { var result = await api('update-provider-model', { connectionId: provider.id, model: model }); providers = providers.map(function (item) { return item.id === result.connection.id ? result.connection : item; }); renderProvider(); status('cvProviderStatus', 'Model updated. Your encrypted API key was retained.', 'success'); } catch (error) { status('cvProviderStatus', message(error), 'error'); } finally { setBusy(providerForm, false); } });
